# -*- coding: utf-8 -*-

# Form implementation generated from reading ui file 'optionPage.ui'
#
# Created by: PyQt5 UI code generator 5.15.2
#
# WARNING: Any manual changes made to this file will be lost when pyuic5 is
# run again.  Do not edit this file unless you know what you are doing.


from PyQt5 import QtCore, QtGui, QtWidgets


class Ui_optionPage(object):
    def setupUi(self, optionPage):
        optionPage.setObjectName("optionPage")
        optionPage.resize(495, 241)
        icon = QtGui.QIcon()
        icon.addPixmap(QtGui.QPixmap("resurces/MainTab/Connect.png"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        optionPage.setWindowIcon(icon)
        optionPage.setStyleSheet("color: rgb(0, 0, 0);\n"
"font: 87 12pt \"Gilroy\";")
        self.centralwidget = QtWidgets.QWidget(optionPage)
        self.centralwidget.setObjectName("centralwidget")
        self.ObjectsLabel = QtWidgets.QLabel(self.centralwidget)
        self.ObjectsLabel.setGeometry(QtCore.QRect(10, 10, 151, 21))
        font = QtGui.QFont()
        font.setFamily("Gilroy")
        font.setPointSize(12)
        font.setBold(False)
        font.setItalic(False)
        font.setWeight(10)
        self.ObjectsLabel.setFont(font)
        self.ObjectsLabel.setObjectName("ObjectsLabel")
        self.parcelBox = QtWidgets.QGroupBox(self.centralwidget)
        self.parcelBox.setGeometry(QtCore.QRect(10, 40, 471, 111))
        font = QtGui.QFont()
        font.setFamily("Gilroy")
        font.setPointSize(12)
        font.setBold(False)
        font.setItalic(False)
        font.setWeight(10)
        self.parcelBox.setFont(font)
        self.parcelBox.setObjectName("parcelBox")
        self.label_2 = QtWidgets.QLabel(self.parcelBox)
        self.label_2.setGeometry(QtCore.QRect(10, 30, 121, 21))
        font = QtGui.QFont()
        font.setFamily("Gilroy")
        font.setPointSize(12)
        font.setBold(False)
        font.setItalic(False)
        font.setWeight(10)
        self.label_2.setFont(font)
        self.label_2.setObjectName("label_2")
        self.modeBox = QtWidgets.QComboBox(self.parcelBox)
        self.modeBox.setGeometry(QtCore.QRect(210, 30, 251, 22))
        self.modeBox.setObjectName("modeBox")
        self.modeBox.addItem("")
        self.modeBox.addItem("")
        self.modeBox.addItem("")
        self.label_3 = QtWidgets.QLabel(self.parcelBox)
        self.label_3.setGeometry(QtCore.QRect(10, 70, 151, 21))
        font = QtGui.QFont()
        font.setFamily("Gilroy")
        font.setPointSize(12)
        font.setBold(False)
        font.setItalic(False)
        font.setWeight(10)
        self.label_3.setFont(font)
        self.label_3.setObjectName("label_3")
        self.timeEdit = QtWidgets.QLineEdit(self.parcelBox)
        self.timeEdit.setGeometry(QtCore.QRect(210, 70, 113, 20))
        self.timeEdit.setObjectName("timeEdit")
        self.OkButton = QtWidgets.QPushButton(self.centralwidget)
        self.OkButton.setGeometry(QtCore.QRect(10, 160, 151, 31))
        self.OkButton.setObjectName("OkButton")
        self.applyButton = QtWidgets.QPushButton(self.centralwidget)
        self.applyButton.setGeometry(QtCore.QRect(170, 160, 151, 31))
        self.applyButton.setObjectName("applyButton")
        self.cancelButton = QtWidgets.QPushButton(self.centralwidget)
        self.cancelButton.setGeometry(QtCore.QRect(330, 160, 151, 31))
        self.cancelButton.setObjectName("cancelButton")
        optionPage.setCentralWidget(self.centralwidget)
        self.menubar = QtWidgets.QMenuBar(optionPage)
        self.menubar.setGeometry(QtCore.QRect(0, 0, 495, 25))
        self.menubar.setObjectName("menubar")
        optionPage.setMenuBar(self.menubar)
        self.statusbar = QtWidgets.QStatusBar(optionPage)
        self.statusbar.setObjectName("statusbar")
        optionPage.setStatusBar(self.statusbar)

        self.retranslateUi(optionPage)
        self.cancelButton.clicked.connect(optionPage.close)
        QtCore.QMetaObject.connectSlotsByName(optionPage)

    def retranslateUi(self, optionPage):
        _translate = QtCore.QCoreApplication.translate
        optionPage.setWindowTitle(_translate("optionPage", "Настройки измерений"))
        self.ObjectsLabel.setText(_translate("optionPage", "Объект измерений"))
        self.parcelBox.setTitle(_translate("optionPage", "Борт"))
        self.label_2.setText(_translate("optionPage", "Режим работы"))
        self.modeBox.setItemText(0, _translate("optionPage", "Режим программирования"))
        self.modeBox.setItemText(1, _translate("optionPage", "Режим контактного датчика"))
        self.modeBox.setItemText(2, _translate("optionPage", "Режим самоликвидации"))
        self.label_3.setText(_translate("optionPage", "Время подрыва, мс"))
        self.timeEdit.setText(_translate("optionPage", "5"))
        self.OkButton.setText(_translate("optionPage", "Ok"))
        self.applyButton.setText(_translate("optionPage", "Принять"))
        self.cancelButton.setText(_translate("optionPage", "Отмена"))
